...
Whilst the population of Sumatran Island tigers is declining, there has been an increase in their numbers in national parks across Sumatra as a result of their degraded habitats elsewhere [(Luskin et al. 2017)](https://doi.org/10.1038/s41467-017-01656-4). Therefore, this project aims to present the tiger  populations in three national parks in Sumatra that are all declared as [UNESCO world heritage sites](https://whc.unesco.org/en/list/?search=Kerinci+Seblat&order=country): Gunung Leuser, Bukit Barisan Selatan and Kerinci Seblat.
...
# **Research Aims**
Research aims
The primary aim of this project is to demonstrate the number and gender of Sumatran tigers inhabiting the three main national parks of Sumatra Island.
...
tmap_arrange(map_2, map_3, #Group map_2 and map_3 into one view
...
# Data Visualisation 2: Zoomed in Map of Sumatra highlighting The Number And Gender Of tigers Spotted In Each National Park
...
# Data Visualisation Number 3: Pie Chart Showing The Gender Proportions Of Sighted Across All National Parks
...
# I have been able to generate an interactive leaflet map of Sumatra
#I was going to use this as my main visualisation but unfortunately, neither my PC nor those at the University had sufficient RAM storage (32GB) to do the final part and knit it to HTML, however, I was able to generate it in both the r notebook and rmarkdown, so I thought I would include it in the code, as I feel it adds to the research questions and demonstrates exactly what I wanted to achieve with this project.
...
map_4 <- tm_shape(IDN_shapefile, bbox= bbox_new) + #add new boundary to the whole shapefile
  ...
  tm_shape(geocode_map)+
 
  tm_markers(col="red", size=0.3)
 

  map_leaflet <- tmap_leaflet(map_4) %>%
  addTiles() %>%
    addCircleMarkers(data= geocode_map,
                     color= "red",
                     radius= 0.3,
